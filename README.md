...
- Node.js `>=20.10.0`（项目提供 `.nvmrc`/`.node-version`，推荐使用 `20.16.0`；也完全支持 Node.js 22+ LTS）
- 包管理器：`npm` ≥ 9 或 `pnpm` ≥ 8（在 Node.js 20.x 中默认提供）
- 使用 `nvm`、`fnm`、`Volta` 等工具可快速切换到项目所需版本；安装依赖时若版本不满足，`preinstall` 钩子会给出明确提示

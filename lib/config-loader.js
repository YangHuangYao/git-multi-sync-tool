...
class ConfigLoader {
  ...
  parseConfigContent(content, configPath) {
    ...
    for (const line of lines) {
      ...
      // 解析远程仓库配置
      const remote = this.parseRemoteLine(trimmedLine, lineNumber, configPath, remotes.length);
      if (remote) {
        remotes.push(remote);
      }
    }
  ...
  // 解析单行配置 - 修复参数传递问题
  parseRemoteLine(line, lineNumber, configPath, currentIndex) {
    // 支持多种格式：
    ...
    if (line.includes(' ')) {
      ...
      // 只有URL，自动生成名称
      remoteUrl = line;
      remoteName = this.generateRemoteName(remoteUrl, currentIndex);
    }
